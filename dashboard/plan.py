         ...
         "subs": [
             ...
             {"id": "1.3", "name": "Turn the dials up",
              ...
              "batches": [
                  ...
                  {"n": "2d", "stage": "straight", "runs": "4 x 1500",
                   "code": "queue_14a.py - the straightness bisect. Inserted "
                           "7 Aug 2026 after reading the drift of every run "
                           "on disk, and it says two things. LENGTH DOES NOT "
                           "HELP: 550 iterations drift about 3.2 deg, 1500 "
                           "gives 3.4-5.5, and the one 5500-iteration run "
                           "gives 6.49. AND WE MADE IT WORSE OURSELVES - same "
                           "seed 1301, same 1500 iterations, r1_crab on 5 Aug "
                           "drifts 3.72 deg with its worst robot at 9.8, and "
                           "gc_1301 on 6 Aug drifts 5.20 with its worst at "
                           "21.9. The worst robot MORE THAN DOUBLED. Batch 2c "
                           "landed three things at once and the speed box "
                           "widened after, so there are four suspects and the "
                           "batch that landed them was measuring gait, not "
                           "straightness. One run per suspect, each differing "
                           "from b0_control in exactly one flag: spin share "
                           "to 0, swing target back to 35 mm, nose_dived "
                           "removed (--no-dive-ends, new). even_stance is off "
                           "in all four - it postdates the regression and the "
                           "owner reports it drives worse. The speed box is "
                           "the fourth suspect and it is deliberately NOT "
                           "here: it moved after gc_1301, so it cannot "
                           "explain a regression measured before it, and it "
                           "is the one change the owner actively wants. It "
                           "gets its own batch. 1500 ITERATIONS FROM SCRATCH, "
                           "and the first attempt got this wrong - it ran 750 "
                           "and b0_control trained cleanly, then covered 0.06 "
                           "m in verify. The A-series numbers it was being "
                           "compared against are 500-iteration FINE-TUNES of "
                           "a 5500-iteration policy; `init_from` is the field "
                           "that says so. From scratch on this card, 750 "
                           "iterations covers 0.06 m and 1500 covers 6.2 - so "
                           "1500 it is, which is also exactly what r1_crab "
                           "and gc_1301 were. NOT 500, the other half of the "
                           "same mistake: `veering` is ramped and reaches its "
                           "full -2.0 only AT iteration 500, so every "
                           "straightness probe so far ended at the moment "
                           "straightness started being charged properly. "
                           "Films off - the checkpoints are still written and "
                           "still drivable",
                   "asks": "which of our own changes cost 1.5 degrees, read "
                           "on the WORST robot as well as the mean. The owner "
                           "drives one robot, and the mean has been "
                           "flattering us for days because the robots scatter "
                           "both ways and cancel",
                   "done": "ABANDONED AFTER ONE RUN, and the reason is worth "
                           "more than the batch was. b0_control - today's "
                           "task, 1500 iterations from scratch, seed 1301, "
                           "5000 robots - covered 0.07 m with a speed error "
                           "of 0.347 m/s. gc_1301, the SAME seed, length and "
                           "robot count on the 6 Aug task, covered 6.18 m at "
                           "0.051. Training looked healthy both times: reward "
                           "168 and rising, full episodes, ground_covered "
                           "normal. The robot just learns to stand. TODAY'S "
                           "TASK CANNOT LEARN TO WALK FROM SCRATCH, and "
                           "nothing noticed because every run since 6 Aug is "
                           "a warm start - L1 continued gc, A1 continued L1, "
                           "A2 to A4 chained on. The leading suspect is the "
                           "speed box, widened to -0.45/0.7 after gc: the "
                           "owner found the 0.9 box teaching a freeze from "
                           "the driving seat, and 0.7 may be a slower version "
                           "of the same trap. f1_box35 in batch 2e tests it "
                           "directly. A bisect cannot run in conditions that "
                           "no longer reproduce, so the straightness work "
                           "moves onto warm starts - which is what every "
                           "policy the owner has ever driven already was."},
                  {"stop": "from-scratch training is broken, so the bisect "
                           "moves to warm starts. DONE 7 Aug - batch 2e."},
                  {"n": "2e", "stage": "straight", "runs": "5 x 750, 1 x 1500",
                   "code": "queue_14b.py - the same question, asked of the "
                           "policy that actually walks. Every arm is "
                           "A1_flat_1301 plus 750 iterations with ONE thing "
                           "changed, so each is comparable to the others AND "
                           "to a robot the owner has driven and called "
                           "crooked (5.33 deg mean, 15.3 on its worst robot). "
                           "w0_control changes nothing and is not optional - "
                           "without it an arm that improves cannot be told "
                           "from a policy that simply trained longer. "
                           "w1_nospin, w2_oldswing and w3_nodive switch off "
                           "the three levers batch 2c landed. w4_veerhard is "
                           "the owner's own pick, go at heading error "
                           "directly: `veering` is RAMPED and only reaches "
                           "its full -2.0 at iteration 500, so this arm "
                           "starts it there and takes it to -4.0. Its risk is "
                           "named in advance - a robot charged heavily for "
                           "heading error can pay zero by not moving, so read "
                           "distance before drift on that one. f1_box35 is "
                           "not a straightness arm at all: 1500 from scratch "
                           "at the OLD +/-0.35 box, the one test that names "
                           "the freeze above. Films off; every checkpoint is "
                           "still drivable",
                   "asks": "which lever costs straightness, whether charging "
                           "heading error harder buys any, and whether the "
                           "speed box is what broke from-scratch training",
                   "done": "NO LEVER IS THE CAUSE, and a measurement taken "
                           "beside the batch says what is. The four arms sit "
                           "within a degree of the control - w0 5.48 deg, w1 "
                           "5.02, w2 4.82, w3 4.79 - which is the same "
                           "not-a-result that A1 to A4 gave. w4_veerhard is "
                           "the exception and it is the owner's own pick: "
                           "4.28 deg mean and 11.26 worst against 5.48 and "
                           "14.06, with distance held at 6.42 m. Its named "
                           "risk - a robot charged -4.0 for heading error can "
                           "pay nothing by standing still - did not happen. "
                           "THE CAUSE, from verify --vary-only on w0 with no "
                           "training at all: freeze every world dial and the "
                           "drift falls to 2.60 deg mean, 5.1 worst, with 63 "
                           "of 64 robots leaning the SAME way. Leave one dial "
                           "free at a time and only one matters - foot grip "
                           "2.86, gearbox drag 2.99, servo strength 2.98, how "
                           "heavy 3.54, and WHERE THE WEIGHT IS 4.90 with its "
                           "worst robot at 11.4, against 4.77 and 12.2 for "
                           "all five together. The centre of mass alone "
                           "reproduces the whole spread. That is why five "
                           "reward attempts in two days moved nothing: the "
                           "robot is not walking crookedly, it is "
                           "compensating - differently every attempt - for a "
                           "trunk balance point it is never told, and no "
                           "reward term can fix an input the policy has not "
                           "got. It is also GOOD NEWS for the real robot, "
                           "which has one centre of mass and therefore a "
                           "fixed lean rather than a wander. A fixed lean "
                           "trims out."},
                  {"stop": "the cause is named, so the question becomes what "
                           "to do about it. DECIDED 7 Aug: narrow the "
                           "centre-of-mass draw in training, since the owner "
                           "is remodelling the CAD with the pot mounts in it "
                           "and the real robot has one balance point - batch "
                           "2f."},
                  {"n": "2f", "stage": "straight", "runs": "5 x 750, 1 x 1500",
                   "code": "queue_14c.py - act on the measurement. "
                           "n1_narrowcom puts the centre-of-mass draw back to "
                           "the narrow range in TRAINING, which is the one "
                           "lever the numbers point at. n2_veer4507 repeats "
                           "w4_veerhard on a second seed, because one seed is "
                           "not a result and this week has read winners off "
                           "single seeds three times. n3_both stacks them. "
                           "s1_fast takes the speed box to 0.9 m/s - 3.2 "
                           "km/h, the owner's FIRST priority - warm-started "
                           "from a policy that already walks, because 0.9 "
                           "taught a from-scratch policy to freeze once "
                           "already; read DISTANCE before anything else on "
                           "that one. t1_mixed puts every ground at once on "
                           "top of the flat policy. f1_box35 runs last: 1500 "
                           "from scratch at the old +/-0.35 box, the test "
                           "that names the from-scratch freeze of batch 2d",
                   "asks": "does narrowing the centre of mass halve the worst "
                           "robot, does the heading charge hold on a second "
                           "seed, how fast can it go, and does it keep it on "
                           "the hills",
                   "done": "THREE IN, ONE OUT, and the one that is out is the "
                           "one this batch was built around. NARROWING THE "
                           "CENTRE OF MASS DOES NOT PAY: on the nominal body "
                           "n1_narrowcom scores 2.42 deg against the "
                           "control's 2.60 - no gain worth having - while on "
                           "the full range it costs 17.4 deg worst and 28.7 "
                           "sideways against 14.1 and 16.4. The policy is "
                           "already near its best on ANY single body; it just "
                           "does not know which body it has, and narrowing "
                           "the draw does not tell it, it only removes the "
                           "practice. The dial stays wide. THE HEADING CHARGE "
                           "HOLDS: 4.28 deg on seed 1301 and 4.38 on 4507, "
                           "against 5.48 - about 1.1 deg on two seeds, with "
                           "distance kept. It does NOT move the worst robot "
                           "(11.26 against 14.65 across the two), which is "
                           "the centre-of-mass problem and not a reward one. "
                           "SPEED IS FREE: the box at 0.9 m/s tracks better "
                           "at every rung than 0.7 did - 0.72 m/s achieved "
                           "when told 0.8, against 0.66, and 94 percent of "
                           "command at 0.7 against 88 - with drift and "
                           "sideways drift both BETTER, not worse. HILLS PAY "
                           "ON HILLS: t1_mixed survives a 10 deg slope 97 "
                           "percent against 88 and climbs 3.80 m against "
                           "2.48, for 0.8 deg of flat forward drift. Neither "
                           "policy makes the 5 m bar on a slope: it does not "
                           "fall off the hill, it climbs it slowly - the same "
                           "finding batch 3 made. AND THE FREEZE IS NAMED: "
                           "f1_box35, from scratch at the old +/-0.35 box, "
                           "covers 6.47 m where b0_control at -0.45/0.7 "
                           "covered 0.07. The speed box is what broke "
                           "from-scratch training, so the box must be GROWN "
                           "and not set - start narrow, warm-start, widen. "
                           "That is also the route to the owner's 3.2 km/h."},
                  {"stop": "the settings that survive become task defaults "
                           "and the final locomotion run is built from them. "
                           "IN: veering to -4.0, speed box to 0.9, mixed "
                           "ground, even_stance off. NARROWING THE CENTRE OF "
                           "MASS WAS CALLED OUT HERE AND THAT WAS WRONG - see "
                           "batch 2g, which measured the pair properly and "
                           "reversed it. x1_combo is all four together, "
                           "warm from A1, for the owner to drive before the "
                           "long run is committed to. MEASURED: x1_combo "
                           "drifts 6.12 deg on the flat - WORSE than "
                           "w4_veerhard's 4.28, because the ground mix costs "
                           "more straightness than the heading charge and the "
                           "speed box buy back - but on a 10 deg slope it "
                           "covers 5.49 m at 98 percent alive, against 3.80 m "
                           "for t1_mixed and 2.48 m at 88 percent for the "
                           "flat control. It PASSES the 5 m bar on a hill, "
                           "which nothing before it has. That is not a "
                           "regression, it is a trade: 1.8 deg of flat "
                           "straightness for a robot that climbs rather than "
                           "merely staying upright. x2_slope7 asks whether "
                           "one gentle 7 deg slope buys most of the climb for "
                           "less of the straightness. MEASURED: it is a real "
                           "middle. Flat drift 5.52 deg against x1's 6.12 and "
                           "w4's 4.28; on a 10 deg slope 4.99 m climbed at 98 "
                           "percent alive against x1's 5.49 m, and it HOLDS "
                           "ITS LINE on the hill - 6.64 deg, inside the bar, "
                           "where x1 gives 7.28 and misses it. Its weakness "
                           "is the worst robot on the flat, 18.90 deg, the "
                           "poorest of the three, on one seed. The three "
                           "candidates are w4_veerhard for a flat robot, "
                           "x2_slope7 for a mostly-flat robot that can manage "
                           "a hill, and x1_combo for a robot that works on "
                           "ground. The owner drives them and picks; the long "
                           "final run is whichever he chooses."},
                  {"n": "2g", "stage": "straight", "runs": "5 x 750",
                   "code": "queue_14b/14c follow-ups plus a new reward term. "
                           "TWO THINGS CAME OUT OF THIS ONE. First, batch 2f "
                           "was WRONG to throw out the narrowed centre of "
                           "mass. It judged n1_narrowcom, which had no heading "
                           "charge, on forward drift alone. y1_flatfast and "
                           "y2_flatfast_narrow differ in nothing but the "
                           "centre-of-mass draw, and the narrowed one gives "
                           "sideways drift 12.75 against 20.33, turn error "
                           "0.157 against 0.214 and turn wander 0.172 against "
                           "0.255, for half a degree of forward drift. The "
                           "same pair appears hours earlier and agrees - "
                           "n3_both 12.22 sideways against w4_veerhard's "
                           "17.10. It is a TRADE, not a rejection: wide for "
                           "the straightest forward walk, narrow for the best "
                           "crabbing and turning, and the two only work "
                           "together - narrowing WITHOUT the heading charge is "
                           "the worst sideways number on record at 28.67. "
                           "Second, `hips_home` - a new reward term written to "
                           "the owner's own curve after he read the per-leg "
                           "hip table. It charges each hip against the CAD "
                           "stance rather than against the other three, which "
                           "is what `even_stance` could never see: three of "
                           "Gray's four legs lean the same way, so the average "
                           "they were being compared against leaned with them",
                   "asks": "does charging an angled hip directly bring the "
                           "legs home, and what does it cost",
                   "done": "IT WORKS, AND MONOTONICALLY, which is what says it "
                           "is real rather than one lucky seed. Hip spread "
                           "between the four legs: 12.6 deg with no term, 8.0 "
                           "at weight -0.5, 5.2 at -1.5. The back-right - the "
                           "leg the owner filmed tucked under the body - goes "
                           "-2.6 deg, -0.5, +0.8: it comes out from under. The "
                           "gap between the widest and tightest foot halves, "
                           "40 mm to 20 mm, and the front-right's worst moment "
                           "falls 24.7 deg to 18.1. Nothing paid for it: at "
                           "-0.5 the drift, worst robot, sideways drift and "
                           "turn error ALL improved on the control. -1.5 has "
                           "the best mean drift, 4.39, and its worst robot "
                           "reads 20.21 against -0.5's 12.91 - one robot in "
                           "64, on the number most polluted by the "
                           "centre-of-mass draw, so that gap is not called "
                           "either way on one seed."},
                  {"stop": "which weight, and does it hold at 9000 robots - "
                           "the new standing batch size, measured by "
                           "scripts/scale_test.py after the renderer came off. "
                           "h2b_hips15_9k and its own control are running."},
              ...
              "batches_note":
                  ...
                  "on a slope. Batch 2d was inserted 7 Aug for the opposite "
                  "reason: the gait batch FIXED the gait and cost "
                  "straightness, which nothing in it was watching. It is a "
                  "bisect rather than a ladder - no new idea is tested, our "
                  "own last four changes are switched off one at a time until "
